    ...
    <div>
      ...
      {doctors.slice(0,3).map((item,index)=>(
        <div className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b" key={index}>
          ...
          <div className="text-sm text-zinc-600 flex-1 self-center ">
          <p className="text-neutral-800 font-semibold">{item.name}</p>
          <p>{item.speciality}</p>
          <p className="text-xs mt-1"><span className="text-sm text-neutral-800 font-medium">Date & Time:</span> 21, October, 2024 | 3.30pm</p>
          </div>
          <div></div>
